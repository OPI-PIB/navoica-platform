...
class CourseDetails(object):
    ...
    def populate(cls, course_descriptor):
        ...
        course_details.difficulty = course_descriptor.difficulty
        course_details.timetable = course_descriptor.timetable
        course_details.organizer = course_descriptor.organizer
    ...
    def update_from_json(cls, course_key, jsondict, user):  # pylint: disable=too-many-statements
        ...
        if 'difficulty' in jsondict and jsondict['difficulty'] != descriptor.difficulty:
            descriptor.difficulty = jsondict['difficulty']
            dirty = True

        if 'timetable' in jsondict and jsondict['timetable'] != descriptor.timetable:
            descriptor.timetable = jsondict['timetable']
            dirty = True

        if 'organizer' in jsondict and jsondict['organizer'] != descriptor.organizer:
            descriptor.organizer = jsondict['organizer']
            dirty = True
